...
class Hangman:
    ...
    def play(self, letter):
        ...
        if letter in self.word_to_find:
            st.session_state.success_message = f"Good guess/ The letter {letter} is in the word."
            for i in range(len(self.word_to_find)):
                if self.word_to_find[i] == letter:
                    self.correctly_guessed_letters[i] = letter
        else:
            st.session_state.error_message = f"Sorry, the letter {letter} is not in the word."
